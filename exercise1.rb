project = {
  ...
  project [:steps].each do |commitee|
    commitee[:person] = projects [:commitee][member]
    member += 1
    if member == 2
      member = 0
    end
    if projects[:steps].each do |commitee|
      commitee[:person] = projects[:commitee].first
      project[:commitee].rotate!
    end
  end
